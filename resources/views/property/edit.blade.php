<x-app-layout>
    <div class="max-w-4xl mx-auto pb-12">
        <div class="mb-8 flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-black text-slate-900 dark:text-white">Edit Property</h1>
                <p class="text-slate-500 dark:text-slate-400 mt-1">Updating details for {{ $property->name }}.</p>
            </div>
        ...
        <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-sm overflow-hidden text-[#1b1b18] dark:text-[#EDEDEC]">
            <form method="POST" action="{{ route('properties.update', $property) }}" class="p-8 lg:p-10 space-y-8">
                @csrf
                @method('PATCH')
